...
def main() -> None:
    ...
    axes[0, 1].scatter(sharpness_steps.numpy(), sharpness.numpy(), s=10)
    ...
    for idx in range(layer_eigs.shape[1]):
        axes[1, 0].scatter(curvature_steps.numpy(), layer_eigs[:, idx].numpy(), s=10, label=f"layer {idx + 1}")
    ...
    for idx in range(layer_gaps.shape[1]):
        axes[1, 1].scatter(curvature_steps.numpy(), layer_gaps[:, idx].numpy(), s=10, label=f"layer {idx + 1}")
    ...
    axes[2, 0].scatter(curvature_steps.numpy(), directional_curv.numpy(), s=10)
    ...
    for idx in range(layer_directional.shape[1]):
        axes[2, 1].scatter(curvature_steps.numpy(), layer_directional[:, idx].numpy(), s=10, label=f"layer {idx + 1}")
    ...
    fig.tight_layout()
    out_path = gd_directory / "gd_combined_metrics_dots.png"
