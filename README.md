...
For example:
```
python src/gd.py cifar10-5k fc-tanh  mse  0.01 100000 --acc_goal 0.99 --neigs 2  --eig_freq 100 --curvature_freq 100
```
...
Every 100 (`eig_freq`) iterations, the top 2 (`neigs`) eigenvalues of the training loss Hessian will be computed and recorded.
Every 100 (`--curvature_freq`) iterations, we compute and record the per-layer maximum eigenvalue, per-layer spectral gap, directional curvature, and per-layer directional curvatures.
...
Within this output directory, the following files will be created, each containing a PyTorch tensor:
 ...
 - `layer_eigs_final`: the per-layer top eigenvalue estimate, measured every 100 (`curvature_freq`) iterations, stored as a matrix of shape `(num_samples, num_layers)`.
 - `layer_eig_gaps_final`: the per-layer spectral gaps (largest minus second-largest eigenvalue), measured also at `curvature_freq`.
 - `directional_curv_final`: the curvature along the update direction, measured also at `curvature_freq`.
 - `layer_directional_curv_final`: the per-layer curvature values along the update direction, measured also at `curvature_freq`.
     By default these tensors are aggregated by module prefix; pass `--layer_granularity tensor` to keep one column per trainable tensor instead.
...
- `curvature_freq` [int, defaults to -1]: the frequency at which we compute layer-wise curvature estimates. If set to a positive integer, the per-layer maxima are saved in `layer_eigs` (and `layer_eigs_final`), the corresponding spectral gaps in `layer_eig_gaps` (and `layer_eig_gaps_final`), and the directional curvature diagnostics in `directional_curv`/`directional_curv_final` and `layer_directional_curv`/`layer_directional_curv_final`. If `curvature_freq` is set to `-1`, none of these quantities are computed.
- `layer_granularity` [string, defaults to `prefix`]: controls how layer-wise curvature values are grouped. With `prefix` tensors sharing a module prefix (e.g. `layer1.weight`, `layer1.bias`) are aggregated together; with `tensor` the values are logged per trainable tensor. The legacy flag `--curvature_grouping` remains as an alias for backward compatibility.
